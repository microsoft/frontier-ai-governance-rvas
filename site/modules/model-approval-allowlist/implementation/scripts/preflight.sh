...
for required in "$register_path" "$template_path" "$parameter_path"; do
  [[ -f "$required" ]] || fail "Required module artifact is missing: $required"
done
...
if register.get("implementationModule") != "model-approval-allowlist":
    raise SystemExit("The register has the wrong implementationModule marker.")
if register.get("implementationSession") != "model-approval-allowlist":

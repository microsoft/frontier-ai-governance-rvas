...
for path in "$deployment_path" "$handoff_path" "$query_path"; do
  [[ -f "$path" ]] || fail "Required Agent 365 module implementation artifact is missing: $path"
done
...
if ((${#unresolved_sentinels[@]} > 0)); then
  ...
  ((${#unknown[@]} == 0)) || fail "Add explicit Agent 365 module preflight checks for new sentinels: ${unknown[*]}"
  fail "Resolve every Agent 365 module decision before changing DLP or Microsoft 365 state: ${unresolved_sentinels[*]}"
fi

[[ $(jq -r '.implementationSession' "$deployment_path") == "optional-module-agent-365-access-boundary" ]] ||
...
[[ $(jq -r '.deployment.action' "$deployment_path") == "InstallAfterDlpPropagation" &&
  ...
  fail "Agent 365 module requires scoped installation after DLP propagation and a scoped-installation removal route."
...
[[ $(jq -r '.dataBoundary.allowedData' "$deployment_path") == "SyntheticOnly" &&
  ...
  fail "Agent 365 module permits labelled synthetic data and scoped access only after DLP propagation."
...
if [[ "$phase" == "dlp" ]]; then
  ...
  echo "PASS: Agent 365 module is ready to configure the scoped DLP policy in TestWithNotifications. This phase does not authorize installation."
  exit 0
fi
...
echo "PASS: Agent 365 module accepts the approved scoped installation after recorded DLP propagation and validates the payload-free audit query."

...
Session 09 creates the inventory record. Session 10 sets runtime authorization and tool security.
...
Use the Session 09 API Center default workspace after the Session 10 runtime decision before a
...
authorization, approval, and telemetry stay with Session 10 and the server platform. This module
...
The flow starts with the MCP server record in the Session 09 inventory. The server owner and
security owner complete the Session 10 runtime checks. The API Center configuration owner then
...
- The approved MCP server record exists in Session 09 and has an owner, environment, deployment or
  package, transport, and lifecycle decision.
- Session 10 has completed runtime authentication, authorization, tool, and telemetry decisions.
...
Choose the API Center that already owns the Session 09 inventory. Record its name and region in the
...
`approvedServers` entry for every server that passed the Session 10 runtime decision. Keep the
...
the inventory owner approves that change. Leave the Session 09 inventory and Session 10 runtime
controls in place.

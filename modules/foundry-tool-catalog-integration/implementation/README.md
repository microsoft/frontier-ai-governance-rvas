...
Session 09 establishes the inventory record. Session 10 sets the MCP security boundary. This module
...
discovery, while [Session 10](../../../sessions/10-mcp-tool-security/) runtime controls authorize
...
|---|---|---|---|---|
| Catalog source | The MCP server record in API Center | Reuses the Session 09 inventory and owner metadata | The Foundry Tools private catalog is public preview and portal-led | Microsoft publishes a stable automated catalog-to-project connection API |
...
| Approval | `require_approval` set to `always` | Agent runtimes receive the requirement with the tool metadata | The runtime must still present and enforce the approval interaction | Session 10 approves a different action-specific policy and the runtime supports it |
...
- the MCP server record in API Center from Session 09 is approved and identifies the remote MCP
  server version and deployment;
- the Session 10 owner has approved the exact MCP tool name and the server's authentication path;
...
- the authentication method differs from the Session 10 decision;
- the project connection is missing or belongs to another project; or
- the selected MCP server exposes no exact tool name approved by Session 10.
...
the Session 10 identity and security owners instead of storing a credential in the Toolbox
payload.

#!/usr/bin/env python3
"""Run structured scope validator fixtures."""
...
def main() -> int:
    ...
    fixtures = json.loads(
        (FIXTURE_ROOT / "scope-subsections.json").read_text(encoding="utf-8")
    )

    for case in fixtures["cases"]:
        failures: list[str] = []
        validator.validate_scope_subsections(
            case["text"],
            case["scopeHeading"],
            failures,
        )
    ...
    print(f"Passed {len(fixtures['cases'])} structured scope fixtures.")
    return 0

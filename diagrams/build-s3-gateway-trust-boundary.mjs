...
els.push(text(40, 60, 1500, "A trust boundary is where authority changes. The gateway mediates auth, routing, and policy between callers and AI services — but a design is an assumption until later sessions evidence it.", C.neutral, { size: 14, align: "left" }));
...
const svc = node(els, 700, 250, 220, 110, C.found, "AI services, tools\n& model backends", { titleSize: 15 });
...
const attachments = [
  ["Private connectivity", "ingress/egress scope"],
  ["Hybrid dependencies", "identity · logging · IR"],
  ["Identity boundary", "who starts the action"],
  ["Telemetry coverage", "not telemetry proof"],
];
let ax = 40;
attachments.forEach(([t, s]) => {
  const a = node(els, ax, 470, 220, 78, C.neutral, t, { titleSize: 13.5, sub: s, subSize: 11.5 });
...
const owner = node(els, 1000, 250, 240, 110, C.hero, "Platform-security owner", { titleSize: 14, sub: "accountable for the boundary;\nruntime assurance evidences it in S6", subSize: 11.5 });

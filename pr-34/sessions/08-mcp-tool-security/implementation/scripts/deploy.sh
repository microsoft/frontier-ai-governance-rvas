...
deployment_json=$(az deployment group create --name session08-mcp-tool-security --resource-group "$(jq -r '.resourceGroupName' <<<"$environment_json")" --template-file "$bicep_path" --parameters "apiManagementName=$(jq -r '.apiManagementName' <<<"$environment_json")" --only-show-errors --output json 2>&1) || fail "Session 08 APIM deployment failed.\n$deployment_json"
[[ $(jq -r '.properties.provisioningState // empty' <<<"$deployment_json") == 'Succeeded' ]] || fail 'Session 08 APIM deployment did not reach Succeeded.'

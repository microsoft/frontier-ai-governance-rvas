# Practical workshop: trace one data path
...
**Customer decision:** Can this one data path continue with known
...
2. **Draw the data path.** Record source, prompt/input, retrieval context, tool
   request, tool response, final response, logs/telemetry, evaluation reuse,
   downstream sharing, evidence reference, and investigation route. Defer any
   source, region, tenant boundary, owner, or minimization point that cannot be
   named.
3. **Classify each segment.** Check customer-approved Purview sensitivity-label
   records, classifier/DSPM findings, source ACL review, or an approved
   alternate classifier record. Treat empty results as "scope reviewed" only when
   workload support, time range, source, role, license, query/scope, and reviewer
   are recorded.
4. **Inspect DLP readiness without changing policy.** Record whether DLP is
   existing, report-only/simulation, enforced, designed, unavailable, not
   applicable, or blocked for each scenario entry point. Do not claim DLP
   coverage for unsupported workloads, conditions, connectors, roles, regions, or
   licenses.
5. **Trace investigation, audit, eDiscovery, and retention.** Name the audit
   search route, eDiscovery or legal-hold owner, retention or records-management
   owner, privacy route, and approved evidence location. Defer when source
   content, AI interaction logs, transcripts, evaluation data, or workshop
   evidence do not have a retention expectation.
6. **Place minimization and identify bypasses.** Record source filters,
   retrieval filters, app redaction, gateway masking, output checks, telemetry
   minimization, and known bypasses such as streaming, direct service calls,
   tool-response payloads, cached retrieval, and evaluation reuse.
7. **Separate gateway and Purview dependencies.** Record what the gateway can
   observe or mask, what Purview governs, and what remains at the source or
   application boundary. A gateway-visible prompt is not proof that source-system
   classification, DLP, audit, retention, or legal hold controls are established.
8. **Record the outcome and handoff.** Approve only when the data path,
   classification/DSPM finding, DLP posture, investigation route, retention
   expectation, minimization point, gateway/Purview dependency, defer criteria,
   receiving owners, and acceptance checks are complete. Otherwise defer with
   owner and target date, reject unsafe use, or route to privacy, legal, data,
   platform, or security operations.

## Workshop artifact

| Artifact field | Capture prompt |
|---|---|
| Scenario scope | Which pilot, workload, environment, user group, and business purpose are in scope? |
| Path trace | What is the source -> prompt -> retrieval -> tool request -> tool response -> final response -> log/evaluation/share route? |
| Segment controls | Which classification, DSPM/exposure, DLP, audit/eDiscovery, retention, residency/privacy, minimization, and gateway/runtime control applies to each segment? |
| Outcome semantics | Is each control result, validated no-result, unsupported, blocked, not applicable, or out of scope? |
| DLP readiness | Which entry points are covered, report-only/simulation, enforced, designed, unavailable, not applicable, or blocked? |
| Investigation route | Which audit, eDiscovery, legal hold, privacy, Insider Risk, Communication Compliance, service log, or records route receives a concern? |
| Minimization and bypasses | Where is data reduced earliest, and which direct, streaming, tool, cache, telemetry, or evaluation bypass remains? |
| Decision | Approve, defer, reject, route, or blocked with owner, accepted-when condition, target date, handoff, and review trigger. |
...
| Production traces or prompts are proposed for evaluation data | Evaluation-data gap | Route to data/compliance owner until de-identification, reuse limit, retention, dataset owner, and deletion route are recorded. |
| Tool response or streaming output bypasses gateway masking | Runtime bypass gap | Move minimization to source, retrieval, application, or downstream API owner; do not claim gateway coverage for unobserved traffic. |
...
|---|---|---|---|---|---|---|
| Pilot decision | Microsoft Purview Data Security Posture Management, Data Loss Prevention, sensitivity labels, audit, eDiscovery, retention, and customer-approved source records | Data governance owner | Customer-approved record reference only | Data path, segment controls, classification/DSPM finding, DLP posture, audit/eDiscovery owner, retention expectation, minimization point, bypasses, gateway/Purview dependency, defer criteria, receiving owners, acceptance checks, exception status, and handoff are complete | Customer date | Compliance operations |

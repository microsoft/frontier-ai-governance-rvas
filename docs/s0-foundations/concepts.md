...
!!! info "Freshness"
    Last reviewed: 2026-07-06 · Shared platform context is in the [Reference landscape](../reference/index.md).
...
**Common misconception:** an operating model is not a slide deck. It becomes useful only when the customer records real owners, decisions, and follow-up actions in the durable artifacts.
...
The value is the conversation behind the score. A low score can reveal a missing owner, absent evidence, or an untested control; a high score should be supported by proof, not confidence. S6 repeats the same instrument so the customer can demonstrate movement and turn remaining gaps into an owned backlog.

**Common misconception:** the baseline is not an audit verdict. It is a prioritization tool that helps the customer choose the next session and measure whether the work had an effect.
...
S0 makes these gaps visible before the technical sessions begin. The use-case intake and risk-classification stub should capture intended capability, data exposure, human oversight, and the accountable sponsor—not merely the product name.
...
S0 does not require the platform to be deployed. Instead, it gives the customer a way to decide who will own the platform evidence and exceptions once it exists. See [Reference Architectures](../reference/reference-architectures.md) for the technical mapping.

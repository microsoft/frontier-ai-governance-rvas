...
| S6 · Runtime Path Evidence & Response | Secure | Runtime enforcement decision with acceptance evidence. | Prompt firewall, APIM policy, Content Safety, correlation, and response skeletons. | `reference-implementations/s6-prompt-firewall/`, `policies/apim/`, `dashboards/azure-monitor/`, `playbooks/sentinel/` | Non-production request path produces expected policy and response signal. | Security owner, SOC owner, and telemetry owner. |

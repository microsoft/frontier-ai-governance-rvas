...
!!! warning "Verify before adopting"
    Last reviewed: 2026-07-30 · This is a Microsoft Learn-aligned LLMOps operating model. Verify current Microsoft Foundry, Azure Monitor, Application Insights, Azure API Management, CI/CD, region, quota, and customer requirements before implementation.
...
and outer-loop model, but make it concrete by inspecting one Foundry candidate
and one customer pipeline/change reference.
...
![S11 illustrative LLMOps change control flow: change card, seven-stage lifecycle, version contracts, release manifest, rollout/fallback/rollback authority, feedback-to-curation gate, automation readiness, blocked gaps, and safe evidence boundary.](../assets/diagrams/s11-llmops-change-control-flow.svg)

## Foundry and pipeline inspection workflow

Use read-only inspection. The safe activity is to compare the baseline and
candidate release references; do not move production traffic, switch aliases,
activate fallback, edit prompts, mutate data, or approve release.

| Step | Open / inspect | Check | Expected signal |
|---|---|---|---|
| Foundry project assets | `ai.azure.com` -> project -> apps/agents/assets. | Workload/app/agent reference, owner, prompt/instruction reference, retrieval/tool reference, deployment alias, evaluation, monitoring link. | Manifest complete, missing owner, unsupported asset, or stale reference. |
| Prompt / instruction version | Foundry prompt flow, agent instruction view, or customer source repo. | Baseline version, candidate version, change summary, owner, rollback reference, excluded content boundary. | Version matched, changed with owner, unversioned prompt, or rollback target missing. |
| Model deployment / alias | Foundry or Azure OpenAI deployments. | Baseline alias, candidate alias, fallback alias, model family/version, region, capacity/quota owner, support boundary. | Alias match, alias mismatch, fallback missing, capacity gap. |
| Dataset / scenario | Foundry evaluation data, scenario asset, or customer evaluation store. | Dataset/scenario reference, source/provenance, split, privacy/retention route, unsupported slices. | Scenario ready, evaluation missing, unsupported slice, or privacy route gap. |
| Evaluation run | Foundry evaluations / agent evaluators or pipeline-generated evaluation output. | Baseline/candidate comparison, evaluator/rubric version, threshold owner, result reference, failed/skipped dimensions. | Passed, failed, missing, diagnostic-only, or threshold owner missing. |
| Release manifest | Customer manifest or generated release metadata. | Prompt/instruction, retrieval, tool schema, model alias, dataset/scenario, evaluation, runtime control, monitoring, rollout, rollback, fallback, approver. | Manifest complete, stale, incomplete, or approver missing. |
| Pipeline run | Azure DevOps/GitHub/customer CI/CD run. | Build/test/eval gates, environment, generated manifest, manual approval, skipped/failed steps, release reference. | Run passed, failed, skipped, blocked, or not tied to manifest. |
| Rollout stage | Release/change record or rollout plan. | Stage, entry conditions, excluded population, stop condition, switch authority, review date. | Ready, hold, unsupported stage, or switch authority missing. |
| Rollback / fallback route | Release manifest, gateway route, alias plan, or operations runbook. | Rollback target, fallback trigger, fallback capacity, compatibility, recovery owner, manual override. | Rollback ready, target missing, fallback unapproved, or capacity gap. |
| Monitoring signal | Foundry observability, Application Insights, Log Analytics, Azure Monitor alert/workbook. | Signal owner, query/alert reference, retention, population, stop/fallback/rollback interpretation. | Monitoring ready, sampled, missing, or cannot inform rollback. |

## Safe baseline-versus-candidate activity

Compare these fields without deploying anything:

| Compare | Baseline | Candidate | Pass condition |
|---|---|---|---|
| Prompt/instruction | Current approved version reference. | Candidate version reference. | Candidate is versioned, owned, and has rollback reference. |
| Model alias | Current alias/deployment reference. | Candidate alias/deployment reference. | Alias movement is explicit and switch authority is named. |
| Dataset/scenario | Baseline evaluation scenario. | Candidate scenario or unchanged reference. | Scenario is approved or unchanged; unsupported slices are listed. |
| Evaluation | Baseline result and threshold. | Candidate result and threshold. | Candidate has result reference or is blocked as evaluation missing. |
| Release manifest | Current release reference. | Candidate manifest reference. | Manifest joins all required version and route references. |
| Pipeline run | Current release pipeline reference. | Candidate run reference. | Required gates passed or failed/skipped gates are routed. |
| Rollout | Current stage and population. | Candidate stage and population. | Stage entry and stop conditions are present. |
| Rollback/fallback | Current rollback and fallback route. | Candidate rollback and fallback route. | Target, trigger, capacity, and owner are named. |
| Monitoring | Current stop/fallback signal. | Candidate monitoring signal. | Signal can inform stop, fallback, rollback, or promotion. |

## Expected inspection signals

| Signal | Meaning | Route |
|---|---|---|
| Manifest complete | Candidate release reference joins prompt/instruction, retrieval, tool schema, model alias, dataset/scenario, evaluation, runtime control, monitoring, rollout, rollback, fallback, and approver. | Ready for separate change-review check. |
| Evaluation missing | Candidate lacks approved evaluation result or only has diagnostic output. | Evaluation owner; block promotion readiness. |
| Alias mismatch | Manifest, Foundry deployment, gateway, or pipeline disagree on baseline/candidate/fallback alias. | Platform/change owner and switch authority. |
| Rollback target missing | Candidate cannot name prior release, alias, route, prompt/retrieval bundle, tool schema, model deployment, or manual hold state. | Release/change owner; block rollout readiness. |
| Feedback source not approved | Feedback lacks privacy/retention route, curation owner, sampling rule, or mutation gate. | Data/privacy and LLMOps owner. |
| Automation not ready | Regression, canary, alias switching, fallback, feedback curation, or retirement lacks evidence, owner, stop condition, or manual override. | Keep manual and route prerequisite. |
| Ready for separate change review | All required references are present, owners are named, stop/fallback/rollback are inspectable, and gaps are explicitly owned. | Customer change/release process decides; S11 does not approve production. |
...
| Affected version references | Safe references to prompt, retrieval config, tool schema, model alias, dataset/scenario, rubric, code release, gateway route, telemetry signal, or feedback queue. |
...
| Version owners | Data, experiment, engineering, evaluation, platform/change, operations, LLMOps, and governance owners as applicable. |
...
| Experimentation | Protected source repo, reproducible candidate reference, Microsoft Foundry project or customer experiment record where supported. | Hypothesis, candidate reference, parameters or prompt-change reference, population, experiment owner, cost/capacity assumption, limitations. | Candidate is worth evaluating, abandoned, or needs a new data/control/engineering action. |
| Evaluation | Microsoft Foundry evaluators/agent evaluators where supported, manual rubric, load/performance evidence where relevant. | Scenario/dataset reference, evaluator or rubric version, threshold owner, baseline/candidate comparison, unsupported slices, result reference. | Candidate has the required comparison, remains diagnostic-only, or is routed for iteration/rejection. |
...
| If the change affects... | Treat it as... | Required checks before next step |
|---|---|---|
...
| Prompt, retrieval, tool behavior, model behavior, fine-tuning, or application configuration | Inner-loop candidate | Experiment record, version contract, evaluation result, release manifest update if advancing. |
...
Use a release manifest to join version references without copying prompts,
...
| Versioned item | Version contract | Reapproval trigger |
|---|---|---|
...
| Candidate | Hypothesis, version references, experiment lineage, scenario/evaluation route, threshold owner, cost/latency/capacity assumption, excluded slices. | New model ID or prompt branch with no comparison question. |
...
| Deprecated | Replacement reference, dependent workloads, support end date, user impact, exception owner, retirement target event. | Old version kept because nobody knows who uses it. |
| Retired | Removal owner, dependency review, retained-record location, rollback limitation, closure evidence reference, reopen trigger. | Deleted version with no dependency or investigation trail. |
...
| Review date | Date or trigger for re-checking stage, exception, capacity, retired versions, or automation eligibility. |

The rollout record can align to `contracts/rollout-decision.schema.json`, whose
...
that the inspected release references are ready for a separate customer change or release
...
| Not ready | Required version reference, owner, evidence reference, route, or stop condition is missing. |
...
| Candidate reference | Dataset, scenario, prompt, retrieval, model, or tool-change reference created from the curated input. |
...
|---|---|---|
| Regression test or scenario comparison | Versioned scenario set, baseline, candidate reference, threshold owner, result location, failure behavior. | Evaluation owner and release/change owner. |
...
| Feedback-to-curation | Feedback source, privacy route, curation rule, candidate reference owner, evaluation route, mutation gate. | Data/privacy owner and LLMOps owner. |
...
|---|---|
| No approved records location | Block. Do not complete the inspection record. |
| Unknown version owner | Defer or block until the owner accepts the record. |
...
|---|---|---|
| Foundry inspection | Project asset, prompt/instruction version, model deployment/alias, dataset/scenario, evaluation run, and monitoring signal are inspected or each gap has an owner and route. | Foundry / LLMOps owner |
| Pipeline inspection | Pipeline run, gates, target environment, generated manifest, skipped/failed steps, and approval state are checked. | Release engineering owner |
| Baseline/candidate comparison | Prompt/instruction, alias, dataset/scenario, evaluation, manifest, rollout, rollback/fallback, and monitoring references are compared without traffic movement. | Change owner |
| Release manifest | Required references are present or the missing reference blocks readiness. | Release/change owner |
| Rollback and fallback | Target, trigger, capacity/compatibility note, switch authority, and recovery owner are named. | Platform / operations owner |
| Feedback gate | Feedback source has privacy/retention route, curation owner, sampling rule, and mutation gate. | Data/privacy owner |
| Automation readiness | Regression, canary, alias switch, fallback, feedback curation, or retirement automation is ready or explicitly remains manual. | LLMOps automation owner |
| Separate change review | The record says ready, ready with owned gaps, defer, route, reject, or block; it does not approve production. | Customer change owner |

## Related references

- [S2 data path controls](../s2-data-compliance/technical.md), [S4 build-path review](../s4-agent-engineering/technical.md), [S7 evaluation evidence](../s7-evaluation/technical.md), and [S10 operating review](../s10-operate-measure/technical.md).
- [Platform technical guide](../reference/platform-technical-guide.md).
- [Microsoft platform governance playbook](../reference/microsoft-platform-governance-playbook.md).

## Boundary note

S11 joins safe references into an LLMOps change inspection record. It selects no

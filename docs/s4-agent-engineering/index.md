...
- A configuration backlog for the selected path, including owners and later-session
  handoffs.
...
release, and production-approval processes.
...
result or no result, limitations, decision, owner, next review, later-session
...
- [ ] The selected-path backlog records configuration decisions, owners, evidence
  references, later-session routing, and customer change-process dependencies.

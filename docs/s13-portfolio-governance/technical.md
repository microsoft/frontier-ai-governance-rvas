...
7. Define roadmap actions and owner readiness.
8. Define baseline feedback triggers.
9. Decide continue, pause, retire, fund, defer, route, or block.

![S13 portfolio decision flow: portfolio review card, source-lineage and coverage package, scorecard, exception concentration, dependency clusters, prioritization, roadmap action, baseline feedback trigger, blocked gaps, and safe evidence boundary.](../assets/diagrams/s13-portfolio-to-s0-feedback-loop.svg)

## Portfolio review card

| Field | Record |
|---|---|
| Portfolio slice | Pilot cohort, agent population, capability group, business unit, region, platform path, roadmap item set, exception set, dependency cluster, or cost/capacity scope. |
...
| Decision forum | Customer forum or owner process that can act on the recommendation. |
...
| Roadmap and baseline | Roadmap, funding/sequencing record, owner-readiness register, foundation baseline questions. | Target date, owner, funding status, dependency, baseline trigger, recheck forum. |

## Scorecard field record

Each scorecard field uses the same field shape:

| Field | Record |
|---|---|
| Scorecard field | Coverage, residual risk, assurance, operating health, cost/capacity, maturity, exception age, dependency, roadmap, owner readiness, or confidence. |
...
| Maturity | Baseline score, current evidence status, movement rationale, blocked domains, confidence. | Supports movement/no-movement/revisit questions, not certification. |
...
| Roadmap | Initiative, owner, target date, funding/capacity status, decision forum, next review trigger, blocked-by list. | Converts portfolio learning into owner-actionable work. |
| Owner readiness | Accountable owner, implementation owner, evidence owner, funding owner, receiving forum, acceptance test. | Blocks unowned work from becoming fake roadmap. |
...
| Escalation route | Forum or authority if unresolved. |
...
| Blocked roadmap items | Safe references only. |
...
| Review trigger | Date or event for rechecking the cluster. |

## Prioritization mechanics

Use weights only when the governance forum owns them. Scores are decision aids,
not proof of value, risk reduction, funding approval, or maturity movement.
...
| Dependency leverage | 1-5 | Higher when item unblocks multiple teams, roadmap items, or evidence routes. |
| Maturity movement | 1-5 | Higher when movement can be supported by evidence against a stable baseline question. |
...
priority_score =
  ...
  (maturity_weight * maturity_movement) +
...
sensitivity, and decision forum. Do not use the formula as automatic ranking,
funding, or compliance logic.

## Roadmap action record

| Field | Record |
|---|---|
| Roadmap action | Continue, pause, retire, fund, defer, route, block, monitor, re-baseline, or open policy question. |
| Action rationale | Risk/value/cost/capacity/coverage/dependency/maturity reason. |
...
| Next review trigger | Date, event, evidence update, exception expiry, or baseline review. |

## Baseline feedback record

| Trigger | Baseline question |
|---|---|
| Repeated ownership gap | Do decision rights, RACI, escalation path, or forum cadence need to change? |
...
| Matured control with stable evidence | Can maturity movement be recorded with supporting references and next cadence? |
...
The record proposes a question, owner, evidence references, target forum,
accepted-when condition, and review trigger. It does not update policy or the
baseline by itself.
...
| Missing source lineage for a key metric | Defer or mark the scorecard field unusable. |
...
| Unowned roadmap action | Defer or block until owner assignment is accepted. |
...
| Maturity movement lacks stable baseline/evidence | Record no movement or unresolved assessment. |

## Acceptance tests

| Work item | Accepted when... | Receiving owner |
|---|---|---|
| Portfolio review card | population, review period, included/excluded scope, decision forum, owners, records location, evidence limits, and stop condition are recorded | Portfolio owner |
| Source-lineage package | every scorecard field or claim links to source owner, freshness date, coverage limit, interpretation owner, and safe reference | Evidence owner |
| Scorecard | field families include coverage, risk, assurance, operating, cost/capacity, maturity, exceptions, dependencies, roadmap, owner readiness, and confidence where relevant | Portfolio/governance owner |
...
| Dependency cluster | dependency type, blocked items, sequence owner, first unblock action, downstream effect, and review trigger are recorded | Roadmap owner |
| Prioritization | weights, rationale, rejected alternatives, uncertainty, decision forum, and action implication are recorded | Governance forum |
...
- [S13 Concepts](concepts.md): portfolio decision view, source lineage, scorecards, exception concentration, dependency clusters, prioritization, and baseline feedback.
...
S13 creates a portfolio decision package and backlog. It creates no dashboard,

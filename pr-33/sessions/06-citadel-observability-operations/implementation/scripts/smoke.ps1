...
function New-SmokeHeaders {
    ...
    if ($BearerToken.Contains("`r") -or $BearerToken.Contains("`n")) {
        throw "CITADEL_SMOKE_BEARER_TOKEN cannot contain a line break."
    }
    $headers = @{
        Authorization = "Bearer $BearerToken"
        "x-citadel-smoke-mode" = $SmokeMode
...
$syntheticMarker = "citadel-probe-" + (-join ((1..32) | ForEach-Object { "{0:x}" -f (Get-Random -Maximum 16) }))
$requiredVariables = @(
    "CITADEL_SMOKE_URL",
    "CITADEL_SMOKE_FAILURE_URL",
    "CITADEL_AI_RESOURCE_ID",
    "CITADEL_LOG_ANALYTICS_WORKSPACE_ID",
...
$pollTimeoutSeconds = Get-BoundedIntegerSetting `
    -Name "CITADEL_SMOKE_TIMEOUT_SECONDS" `
    -Default 180 `
    -Minimum 30 `
    -Maximum 600
$pollRetrySeconds = Get-BoundedIntegerSetting `
    -Name "CITADEL_SMOKE_RETRY_SECONDS" `
    -Default 15 `
    -Minimum 5 `
    -Maximum 60
if (-not (Test-PollTimingValid `
    ...
    throw "CITADEL_SMOKE_TIMEOUT_SECONDS must be at least twice CITADEL_SMOKE_RETRY_SECONDS."
}

if ($env:CITADEL_AI_RESOURCE_ID -notmatch "^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft\.Insights/components/[^/]+$") {
    throw "CITADEL_AI_RESOURCE_ID must be a full Application Insights resource ID."
}
if ($env:CITADEL_LOG_ANALYTICS_WORKSPACE_ID -notmatch "^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft\.OperationalInsights/workspaces/[^/]+$") {
    throw "CITADEL_LOG_ANALYTICS_WORKSPACE_ID must be a full Log Analytics workspace resource ID."
}
if ($env:CITADEL_SMOKE_URL -notmatch "^https://" -or $env:CITADEL_SMOKE_FAILURE_URL -notmatch "^https://") {
    throw "Both smoke endpoints must use HTTPS."
}
if ($env:CITADEL_SMOKE_FAILURE_URL -eq $env:CITADEL_SMOKE_URL) {
...
$component = az resource show `
    --ids $env:CITADEL_AI_RESOURCE_ID `
    --api-version 2020-02-02 `
    --output json | ConvertFrom-Json
if ($LASTEXITCODE -ne 0 -or
    [string]$component.type -ine "microsoft.insights/components") {
    throw "CITADEL_AI_RESOURCE_ID could not be resolved as an Application Insights component."
}
...
if ([string]::IsNullOrWhiteSpace($componentWorkspaceResourceId) -or
    ...
    -ExpectedWorkspaceResourceId $env:CITADEL_LOG_ANALYTICS_WORKSPACE_ID)) {
    throw "The Application Insights component WorkspaceResourceId does not match CITADEL_LOG_ANALYTICS_WORKSPACE_ID."
}
$normalizedWorkspaceResourceId = Normalize-ResourceId `
    $env:CITADEL_LOG_ANALYTICS_WORKSPACE_ID
...
$headers = New-SmokeHeaders `
    -BearerToken $env:CITADEL_SMOKE_BEARER_TOKEN `
    -SmokeMode "normal" `
...
$failureResponse = Invoke-WebRequest `
    -Uri $env:CITADEL_SMOKE_FAILURE_URL `
    -Method Post `
...
if ($result.status -ne "passed") {
    throw "Session 06 smoke checks failed. Inspect the payload-free result at $ResultPath."
}
Write-Host "PASS: Session 06 smoke checks passed."

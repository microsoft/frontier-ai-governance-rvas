...
usage() {
  cat <<'USAGE'
Usage:
  check-spoke.sh --approved-subscription-id SUBSCRIPTION_ID --resource-group-name RESOURCE_GROUP --foundry-account-name FOUNDRY_ACCOUNT --project-name PROJECT --read-api-base-url HTTPS_URL --application-insights-resource-id RESOURCE_ID
USAGE
}
...
grep -q 'Refuse requests to create, update, approve, publish, delete' "$instructions_path" &&
  grep -q 'The prohibited write action is `' "$instructions_path" &&
  grep -q 'Route valid change requests to `' "$instructions_path" ||

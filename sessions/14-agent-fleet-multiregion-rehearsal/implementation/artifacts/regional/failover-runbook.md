...
Move the governed service from the primary selector to the secondary selector in the regional
...
approved change. Both selectors must be present and name different routes.

## Before routing changes

1. Freeze infrastructure and gateway policy changes for the rehearsal window.
2. Run `scripts/preflight.ps1 -Phase Ready` or `scripts/preflight.sh --phase ready`. Inspect the
   live Azure checks and what-if result. The failover wrapper repeats this ready check immediately
...
primary selector to the secondary selector listed in the topology. The script then calls the
customer routing control.

Observe normal service telemetry in Azure Monitor and the relevant service portals. Stop and
...
readiness. Preview the selector restore, get delivery-owner confirmation for the second high-impact
production traffic move, and move the selector listed in this runbook. Then check the primary path.
...
developer portal remain in the primary region. Internal mode needs the customer's cross-region

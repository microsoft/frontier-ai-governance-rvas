...
Move the governed agent from the primary selector to the secondary selector listed in the topology during an
approved maintenance window. The same identity, API Management policy version, agent version, and
trace path must survive the move.
...
- Operational record store: `__REQUIRED_OPERATIONAL_RECORD_STORE__`

Stop if the scope, current release, maintenance window, or restore authority differs from
`control-definition.json`. Both selectors must be present. They must name different routes.
...
4. Check that the primary selector still serves the agent version listed in the topology.
5. Open the customer's normal incident or change record. Do not copy runtime output into this
   repository.
...
Observe the normal service telemetry. The Agent 365, Purview, and Defender inventory file is a
dated operator snapshot, not current-state proof. Stop and restore the primary selector if the secondary path
does not report the expected identity, gateway policy, agent version, and trace.

## Restore the primary selector

Use the same approved scope and the routing control listed in `control-definition.json`. The service
owner confirms primary readiness. Preview the selector restore, get delivery-owner confirmation for
the second high-impact production traffic move, and move only the selector listed in this runbook.
Then check the primary path.

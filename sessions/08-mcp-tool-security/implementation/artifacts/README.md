...
These files define the Session 08 MCP implementation: an APIM MCP server that exposes the
...
| Path | Purpose |
|---|---|
| `apim/main.bicep` | MCP server, its only tool reference, APIM named values, policy, and payload-free diagnostics |
...
| `governance/agent-mcp-binding.json` | Candidate binding, approved-tool contract, prohibited action, and approval settings |

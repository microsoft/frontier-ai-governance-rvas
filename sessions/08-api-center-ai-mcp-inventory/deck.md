...
> Add the Session 05 agent API, synchronize the Session 08 APIM API, and register an approved remote MCP server in API Center.

By the end of the session:

- A tagged API Center holds the direct agent definition and required metadata schema.
- Its managed identity reads the exact Session 08 APIM service.
...
API Center supports discovery. APIM and Session 10 control runtime use.
...
- evaluation destination, review date, expiry date, and Session 09 marker.
...
- The region, APIM tier, Session 08 marker, reader role, and names match.
- The preview changes only the marked API Center scope and exact reader assignment.

The reader role is `71522526-b88f-4d52-b57f-d31fc3546d0d` on the Session 08 APIM service.
...
- The direct agent API, synchronized Session 08 API, and MCP server each appear once.
...
Check the Session 09 marker, remove the exact APIM reader assignment, then delete only the marked API Center.
...
[Session 10](../10-mcp-tool-security/) constrains MCP identities, tools, arguments, outputs, and side effects.

<!-- Notes: Inventory says what exists. Session 10 governs what the tools can do. -->

...
usage() {
  cat <<'USAGE'
Usage:
  deploy.sh --approved-subscription-id SUBSCRIPTION_ID --session06-agent-base-url HTTPS_URL --remote-mcp-server-url HTTPS_URL
USAGE
}
...
"$script_dir/preflight.sh" --approved-subscription-id "$approved_subscription_id" --session06-agent-base-url "$session06_agent_base_url" --remote-mcp-server-url "$remote_mcp_server_url"

...
This record defines the Session 08 APIM gateway routing decision. APIM is authoritative for the
...
| Consumer | The API product owner approves routing changes with this record. The platform owner applies them through `gateway-control.json` and the Session 08 deployment path. |
...
splits the approved workload allowance by region in Session 15. Do not assume a shared global
counter.

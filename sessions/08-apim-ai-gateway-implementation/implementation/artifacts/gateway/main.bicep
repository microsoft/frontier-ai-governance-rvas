...
var implementationSession = '08-apim-ai-gateway-implementation'
                ...
                replace(
                  replace(
                    replace(
                      ...
                      '${openBrace}${openBrace}session08-entra-tenant-id${closeBrace}${closeBrace}'
                    ),
                    '__NAMED_VALUE_CLIENT_APPLICATION_ID__',
                    '${openBrace}${openBrace}session08-client-application-id${closeBrace}${closeBrace}'
                  ),
                  '__NAMED_VALUE_API_AUDIENCE__',
                  '${openBrace}${openBrace}session08-api-audience${closeBrace}${closeBrace}'
                ),
                '__NAMED_VALUE_REQUIRED_APP_ROLE__',
                '${openBrace}${openBrace}session08-required-app-role${closeBrace}${closeBrace}'
              ),
...
resource apiAudience 'Microsoft.ApiManagement/service/namedValues@2024-05-01' = {
  parent: apim
  name: 'session08-api-audience'
  properties: {
    displayName: 'session08-api-audience'
    secret: false
    tags: [
      implementationSession
    ]
...
resource primaryBackend 'Microsoft.ApiManagement/service/backends@2024-05-01' = {
  ...
  properties: {
    ...
    title: 'Session 08 primary Foundry agent backend'
...
resource secondaryBackend 'Microsoft.ApiManagement/service/backends@2024-05-01' = if (secondaryBackendEnabled) {
  ...
  properties: {
    ...
    title: 'Session 08 secondary Foundry agent backend'
...
resource backendPool 'Microsoft.ApiManagement/service/backends@2024-05-01' = {
  ...
  properties: {
    type: 'Pool'
    title: 'Session 08 governed agent backend pool'

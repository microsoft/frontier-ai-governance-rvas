...
# Estate lifecycle sentinels:
# __REQUIRED_APPROVED_ESTATE_SCOPE_ALIAS__ __REQUIRED_APPROVED_MANAGEMENT_GROUP_ID__
# __REQUIRED_APPROVED_PRIMARY_REGION__ __REQUIRED_OWNER_TAG_KEY__ __REQUIRED_COST_TAG_KEY__
# __REQUIRED_ENVIRONMENT_TAG_KEY__ __REQUIRED_EXCEPTION_ACCOUNT_NAME__ __REQUIRED_EXCEPTION_REASON__
# __REQUIRED_EXCEPTION_OWNER_ROLE__ __REQUIRED_EXCEPTION_EXPIRY_DATE__
# __REQUIRED_INVENTORY_OWNER_ROLE__ __REQUIRED_LIFECYCLE_OWNER_ROLE__ __REQUIRED_ESTATE_REVIEW_DATE__
...
Runs the Session 07 Bash preflight. The script validates required files, tools, sentinels, the two
...
Ready-phase options:
  --baseline-record-path <temporary-json-path>         External Session 05 baseline result for Ready preflight.
  --candidate-record-path <temporary-json-path>        External Session 05 candidate result for Ready preflight.
  --security-release-attestation-path <temporary-json-path>
                                                       External Session 05 attestation for Ready preflight.
...
covered_decision_sentinels=(
  ...
  "__REQUIRED_APPROVED_AGENT_FRAMEWORK_PATH__"
  ...
  "__REQUIRED_DUPLICATE_AGENT_REVIEW_DECISION__"
  "__REQUIRED_DUPLICATE_REVIEW_OWNER_ROLE__"
  ...
  "__REQUIRED_FRAMEWORK_EXCEPTION_SUPPORT_OWNER_ROLE_OR_NA__"
  ...
  "__REQUIRED_REVIEWED_AGENT_INVENTORY_REFERENCE__"
...
if control.get('implementationSession') != '07-controlled-promotion-lifecycle':
    raise SystemExit('Session 07 implementation files have the wrong implementationSession marker.')
...
threshold_policy_path = repo_root / control['sourcePaths']['evaluationThresholdPolicy']
if hashlib.sha256(threshold_policy_path.read_bytes()).hexdigest() != control['immutableRelease']['evaluationThresholdPolicySha256']:
    raise SystemExit('The approved Session 05 threshold policy hash does not match immutable release metadata.')
...
smoke_powershell_path = resolve_repo_path(control['sourcePaths']['observabilitySmokePowerShell'])
smoke_bash_path = resolve_repo_path(control['sourcePaths']['observabilitySmokeBash'])
...
for parameters, environment_name in ((nonprod_params['parameters'], 'nonproduction'), (prod_params['parameters'], 'production')):
    if parameters['environment']['value'] != environment_name or parameters['implementationSession']['value'] != '07-controlled-promotion-lifecycle':
...
for required_name in ('CITADEL_SMOKE_URL', 'CITADEL_SMOKE_FAILURE_URL', 'CITADEL_AI_RESOURCE_ID', 'CITADEL_LOG_ANALYTICS_WORKSPACE_ID'):
    if not str(variables['nonproduction'].get(required_name, '')).strip():
        raise SystemExit(f'nonproduction GitHub environment variable {required_name} is required for the Session 06 smoke.')
try:
    poll_timeout = int(str(variables['nonproduction'].get('CITADEL_SMOKE_TIMEOUT_SECONDS', '')).strip() or '180')
    poll_retry = int(str(variables['nonproduction'].get('CITADEL_SMOKE_RETRY_SECONDS', '')).strip() or '15')
except ValueError as error:
    raise SystemExit('Session 06 telemetry polling values must be integers.') from error
if not 30 <= poll_timeout <= 600 or not 5 <= poll_retry <= 60 or poll_retry > poll_timeout:
    raise SystemExit('Session 06 telemetry polling must use timeout 30-600 seconds and retry 5-60 seconds.')
if 'CITADEL_SMOKE_BEARER_TOKEN' not in nonproduction_secret_names:
    raise SystemExit('nonproduction GitHub environment secret CITADEL_SMOKE_BEARER_TOKEN is required.')

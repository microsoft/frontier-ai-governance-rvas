...
foreach ($requiredName in @(
    ...
    if (-not $nonproductionVariables.ContainsKey($requiredName) -or
        ...
        throw "nonproduction GitHub environment variable $requiredName is required for the Session 06 smoke."
    }
}
$pollTimeout = 180
$pollRetry = 15
$pollTimeoutValue = [string]$nonproductionVariables["CITADEL_SMOKE_TIMEOUT_SECONDS"]
$pollRetryValue = [string]$nonproductionVariables["CITADEL_SMOKE_RETRY_SECONDS"]
if ((-not [string]::IsNullOrWhiteSpace($pollTimeoutValue) -and
    ...
    throw "Session 06 telemetry polling must use timeout 30-600 seconds and retry 5-60 seconds."
}
if (@($nonproductionSecretNames) -cnotcontains "CITADEL_SMOKE_BEARER_TOKEN") {
    throw "nonproduction GitHub environment secret CITADEL_SMOKE_BEARER_TOKEN is required."
}
foreach ($pair in @(
...
if (-not $?) {
    throw "Session 05 evaluation dependencies or the external Session 05 security-release attestation are not ready."
}

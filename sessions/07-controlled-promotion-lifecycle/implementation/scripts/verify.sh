...
Runs the Session 07 Bash verification entrypoint. The script validates the requested check, reads the
...
grep -Eq '"implementationSession"[[:space:]]*:[[:space:]]*"07-controlled-promotion-lifecycle"' "$control_path" ||
...
if check == 'intended':
    ...
    if validation_job is None:
        raise SystemExit('The blocked workflow run does not identify the generated Session 05 self-test input.')
    gate_step = next((step for step in validation_job.get('steps', []) if step.get('name') == 'Apply evaluation and adversarial gates before deployment'), None)
    failed_before_gate = [

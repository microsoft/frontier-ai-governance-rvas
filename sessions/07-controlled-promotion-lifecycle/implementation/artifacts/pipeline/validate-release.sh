...
Runs the Session 07 Bash release validator. The script validates approved policy, immutable
...
Optional options:
  ...
  --baseline-record-path <temporary-json-path>                         External Session 05 baseline record for dependencies/intended modes.
  --candidate-record-path <temporary-json-path>                        External Session 05 candidate record for dependencies/intended modes.
  --security-release-attestation-path <temporary-json-path>           External Session 05 attestation for dependencies/intended modes.
...
if report.get('implementationSession') != '05-evaluation-threat-gates':
    raise SystemExit('Session 05 security-release attestation has the wrong implementationSession marker.')
...
if set(report) != required_attestation_fields:
    raise SystemExit('Session 05 security-release attestation has an incomplete or payload-bearing root schema.')
if (
    report.get('schemaVersion') != 1
    or report.get('recordType') != 'security-release-attestation'
    or report.get('status') != 'confirmed'
):
    raise SystemExit('Session 05 security-release attestation must be confirmed; pending or failed attestations block promotion.')
...
if (
    ...
    raise SystemExit('Session 05 attestation must carry authorized external security/change status and report location.')
target = report.get('target') or {}
release_policy = json.loads(Path(state['evaluationReleasePolicyPath']).read_text())
if (
    ...
    raise SystemExit('Session 05 attestation does not bind the approved baseline and remediated release-agent versions.')
if not __import__('re').fullmatch(r'[0-9a-fA-F]{64}', str(report.get('configurationSha256', ''))):
    raise SystemExit('Session 05 security-release attestation must name the shared attack-plan configuration SHA-256.')
...
if (
    ...
    raise SystemExit('Session 05 attestation must confirm its baseline and remediated versions match the release agent.')
...
for run_name in ('baseline', 'postRemediation'):
    ...
    if not isinstance(run, dict) or set(run) != required_run_fields:
        raise SystemExit(f'Session 05 security-release attestation {run_name} result has an incomplete schema.')
    if any(not str(run.get(field) or '').strip() for field in ('evalId', 'runId', 'reportUrl')):
        raise SystemExit(f'Session 05 security-release attestation {run_name} result is missing its native run identifiers or report URL.')
    rate = run.get('overallAttackSuccessRate')
    if isinstance(rate, bool) or not isinstance(rate, (int, float)) or not 0 <= rate <= 1:
        raise SystemExit(f'Session 05 security-release attestation {run_name} overallAttackSuccessRate must be numeric and between zero and one.')
...
if set(privacy) != required_privacy:
    raise SystemExit('Session 05 security-release attestation has an incomplete privacy schema.')
for field in (
    'containsAttackPrompts',
    'containsAgentResponses',
    'containsToolPayloads',
    ...
    if privacy.get(field) is not False:
        raise SystemExit(f'Session 05 security-release attestation must remain payload-free: privacy.{field}.')
...
if comparison.get('lowerOverallAttackSuccessRate') is not True or comparison.get('perRiskNonRegressionPassed') is not True or comparison.get('prohibitedActionsBlocked') is not True:
    raise SystemExit('Session 05 security-release attestation is not in the required confirmed state.')
...
if (
    ...
    raise SystemExit('Session 05 security-release attestation overall attack-success comparison is invalid or did not improve.')
metrics = comparison.get('metrics')
if not isinstance(metrics, list) or not metrics:
    raise SystemExit('Session 05 security-release attestation must include per-risk comparison rows.')
keys = set()
...
for metric in metrics:
    if set(metric) != required_metric_fields:
        raise SystemExit('Session 05 security-release attestation has an incomplete per-risk comparison schema.')
    key = tuple(str(metric.get(field, '')).strip() for field in ('evaluatorName', 'riskCategory', 'attackStrategy'))
    if not all(key):
        raise SystemExit('Session 05 per-risk comparison is missing evaluator, risk category, or attack strategy.')
    if key in keys:
        raise SystemExit('Session 05 security-release attestation has a duplicate per-risk comparison row.')
    keys.add(key)
    if metric.get('nonRegressionPassed') is not True:
        raise SystemExit('Session 05 security-release attestation per-risk comparison contains a regression.')
    ...
    if (
        ...
        raise SystemExit('Session 05 security-release attestation per-risk comparison has invalid or regressed attack-success rates.')
prohibited = [
    metric
    for metric in metrics
...
if not prohibited or any(metric.get('postRemediationAttackSuccessRate') != 0 for metric in prohibited):
    raise SystemExit('Session 05 security-release attestation prohibited-actions metrics must end at zero attack success.')
PY
}
...
if (
    ...
    or smoke.get('implementationSession') != '06-citadel-observability-operations'
    or smoke.get('recordType') != 'citadel-smoke-result'
    or smoke.get('mode') != 'pipeline'
    or smoke.get('environment') != 'nonproduction'
    or smoke.get('status') != 'passed'
    or smoke.get('implementationMarker') != 'implementationSession=06-citadel-observability-operations'
):
    raise SystemExit('Session 06 smoke result has an invalid root contract or non-passing status.')
if smoke.get('commitSha') != release_sha:
    raise SystemExit('Session 06 smoke result does not target the promoted commit SHA.')
correlation_ids = {
...
if any(
    ...
    raise SystemExit('Session 06 smoke result correlation fields must be lower-case W3C trace IDs.')
if (
    ...
    raise SystemExit(
        'Session 06 smoke result must bind its root correlation to distinct normal and failure traces.'
    )
checks = smoke.get('checks', {})
for field in ('syntheticRequest', 'endToEndTrace', 'toolAndModelFailureSeparated'):
    if checks.get(field) != 'passed':
        raise SystemExit(f"Session 06 smoke result check '{field}' did not pass.")
for field in (
    ...
    if checks.get(field) is not True:
        raise SystemExit(f"Session 06 smoke result check '{field}' must be the JSON boolean true.")
for field in ('sensitiveInputPresent', 'payloadsRetained'):
    if checks.get(field) is not False:
        raise SystemExit(f"Session 06 smoke result check '{field}' must be the JSON boolean false.")
if checks.get('privacySurfacesChecked') != [
    ...
    raise SystemExit('Session 06 smoke result must check the five required telemetry privacy surfaces.')
...
if (
    ...
    raise SystemExit('Session 06 smoke result does not show a successful bounded telemetry-ingestion poll.')
if smoke.get('payloadsRetained') is not False:
    raise SystemExit('Session 06 smoke result must report payloadsRetained=false.')
try:
    __import__('datetime').datetime.fromisoformat(str(smoke.get('observedAt', '')).replace('Z', '+00:00'))
except ValueError as error:
    raise SystemExit('Session 06 smoke result must include a valid observedAt timestamp.') from error
PY
}
...
run_evaluation_gate() {
  ...
  release_gate="$(json_get "$state_json" 'evaluationReleaseGatePath')"
  release_policy="$(json_get "$state_json" 'evaluationReleasePolicyPath')"
  thresholds="$(json_get "$state_json" 'evaluationThresholdPolicyPath')"
  spec="$(json_get "$state_json" 'evaluationSpecPath')"
  dataset="$(json_get "$state_json" 'evaluationDatasetPath')"
...
if str(candidate['run']['runId']) != str(control['immutableRelease']['evaluationRunId']):
    raise SystemExit('The passing Session 05 candidate record does not match immutableRelease.evaluationRunId.')
PY
  ...
  if ! python "$release_gate" \
    ...
    fail "Session 05 release gate did not produce expected outcome '$expected'."
  fi
}

run_evaluation_blocked_self_test() {
  local state_json="$1"
  local self_test
  self_test="$(json_get "$state_json" 'evaluationGateSelfTestPath')"
  if ! python "$self_test" --mode blocked-tool-process; then
    fail 'Session 05 generated blocked-tool-process self-test did not return BLOCK.'
  fi
}
...
if runtime.get('implementationSession') != '07-controlled-promotion-lifecycle':
...
if manifest.get('implementationSession') != '07-controlled-promotion-lifecycle':
...
def assert_marker(value: dict, purpose: str, expected: str = '07-controlled-promotion-lifecycle') -> None:
...
threshold_policy_path = resolve_repo_path(paths['evaluationThresholdPolicy'], 'Session 05 threshold policy', ('.yaml', '.yml'))
release_policy_path = resolve_repo_path(paths['evaluationReleasePolicy'], 'Session 05 release policy', ('.json',))
release_policy = read_json(release_policy_path)
assert_marker(release_policy, 'Session 05 release policy', '05-evaluation-threat-gates')
if release_policy.get('schemaVersion') != 2:
    raise SystemExit('Session 05 release policy must use schemaVersion 2.')
...
if release_policy.get('activationContract') != expected_activation_contract:
    raise SystemExit('Session 05 release policy has an unexpected activationContract.')
gate = release_policy.get('gate', {})
if gate.get('callableInterface', {}).get('requiredEnforcementOption') != '--require-enabled':
    raise SystemExit('Session 05 callable release gate must require --require-enabled.')
try:
    decision_date = date.fromisoformat(str(gate.get('decisionDate', '')))
except ValueError as error:
    raise SystemExit('Session 05 release policy needs an approved decision date.') from error
if decision_date > date.today():
    raise SystemExit('Session 05 release policy decisionDate cannot be in the future.')
if (
    ...
    raise SystemExit('Session 05 release policy must be enabled, approved, dated, and bind both run IDs.')
...
if (
    ...
    raise SystemExit('Session 05 release policy must target the approved immutable agent version.')
if (
    gate.get('candidateRunId') != immutable.get('evaluationRunId')
):
    raise SystemExit('Session 05 release-policy candidate run ID must match the immutable release.')
threshold_policy = yaml.safe_load(threshold_policy_path.read_text())
if threshold_policy.get('policy_state') != 'active':
    raise SystemExit('Session 05 threshold policy must be active.')
...
if str(immutable.get('evaluationThresholdPolicySha256')) != threshold_policy_hash:
    raise SystemExit('immutableRelease.evaluationThresholdPolicySha256 does not match the approved Session 05 threshold policy.')

resolved = {
    'releaseSha': release_sha,
    'controlPath': str(control_path),
    'manifestTemplatePath': str(manifest_template_path),
    'evaluationReleaseGatePath': str(resolve_repo_path(paths['evaluationReleaseGate'], 'Session 05 release gate', ('.py',))),
    'evaluationReleasePolicyPath': str(release_policy_path),
    'evaluationGateSelfTestPath': str(resolve_repo_path(paths['evaluationGateSelfTest'], 'Session 05 generated blocked self-test', ('.py',))),
    'evaluationThresholdPolicyPath': str(threshold_policy_path),
    'evaluationSpecPath': str(resolve_repo_path(paths['evaluationSpec'], 'Session 05 evaluation specification', ('.json',))),
    'evaluationDatasetPath': str(resolve_repo_path(paths['evaluationDataset'], 'Session 05 evaluation dataset', ('.jsonl',))),
    ...
    'observabilitySmokePowerShellPath': str(resolve_repo_path(paths['observabilitySmokePowerShell'], 'Session 06 PowerShell smoke script', ('.ps1',))),
    'observabilitySmokeBashPath': str(resolve_repo_path(paths['observabilitySmokeBash'], 'Session 06 Bash smoke script', ('.sh',))),
...
for parameters_doc, environment_name in (
    ...
    if parameters.get('environment', {}).get('value') != environment_name or parameters.get('implementationSession', {}).get('value') != '07-controlled-promotion-lifecycle':
...
case "$mode" in
  ...
  dependencies)
    ...
    baseline_record_path="$(resolve_temporary_external_json "$baseline_record_path" 'Session 05 approved baseline record')"
    candidate_record_path="$(resolve_temporary_external_json "$candidate_record_path" 'Session 05 candidate record')"
    security_release_attestation_path="$(resolve_temporary_external_json "$security_release_attestation_path" 'Session 05 security-release attestation')"
    run_evaluation_gate "$state_json" "$baseline_record_path" "$candidate_record_path" pass
    run_evaluation_blocked_self_test "$state_json"
    assert_security_release_attestation "$state_json" "$security_release_attestation_path"
    printf 'PASS: Session 05 permitted path, generated blocked self-test, and confirmed external Session 05 security-release attestation are ready.\n'
    ;;
  smoke)
    ...
    printf 'PASS: Session 06 smoke and observability result is complete and payload-safe.\n'
    ;;
  intended)
  ...
  blocked)
    run_evaluation_blocked_self_test "$state_json"
    printf 'PASS: the Session 05 generated blocked-tool-process self-test returned BLOCK.\n'
    ;;
  create-manifest)

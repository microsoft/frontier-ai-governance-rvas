...
az deployment group what-if --name session08-mcp-preview --resource-group "$(jq -r '.resourceGroupName' <<<"$environment_json")" --template-file "$bicep_path" --parameters "apiManagementName=$(jq -r '.apiManagementName' <<<"$environment_json")" --only-show-errors --no-pretty-print >/dev/null || fail 'The Session 09 deployment preview failed.'

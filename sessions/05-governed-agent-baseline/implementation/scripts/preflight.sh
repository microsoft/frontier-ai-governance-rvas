...
jq -e '.implementationSession == "06-governed-agent-baseline"' "$config_path" >/dev/null || fail "agent.json has the wrong implementation marker."
jq -e '.agentType == "prompt" and .runtimePattern == "persistent-prompt-agent"' "$config_path" >/dev/null || fail "Session 06 implements one persistent prompt agent."
...
jq -e '.implementationSession == "06-governed-agent-baseline" and .tracing.mode == "server-side" and .tracing.connectionManagedOutsideSession == true' "$release_operations_path" >/dev/null || fail "Tracing must use the existing project connection to Application Insights."
...
[[ $(jq -r '.properties.provisioningState' <<<"$model_json") == 'Succeeded' ]] || fail "The approved Session 05 model deployment is not ready."
...
if (( existing_count == 1 )); then
  ...
  [[ "$description" == *'06-governed-agent-baseline'* ]] || fail "An existing agent uses the configured name but does not carry the Session 06 marker."
...
echo 'PASS: Session 06 files, decisions, live release selector, approved Azure scope, model, Application Insights resource, Foundry project access, read-only tool boundary, unique-identity path, and preview gate are ready.'

...
<!-- Notes: Session 12 connected adversarial behavior to detection. This session adds the operating controls for one service. -->
...
- The Session 14 workflow can run the paired payload-free smoke check.
...
The customer APIM repository owns gateway policy. Session 14 consumes the temporary smoke result.
...
The Session 14 GitHub workflow runs `smoke.ps1` or `smoke.sh` with:
...
<!-- Notes: Session 14 promotes these definitions and consumes the smoke result. -->

...
This example assumes that the Session 03 policy assistant calls a customer-supplied MCP endpoint
through the Session 02 Governance Hub. The sample binding maps `get_policy` to an existing GET
operation. Replace those identifiers with the approved customer operation. This repository does
not deploy the backing API or MCP server.
...
|---|---|---|---|---|
| User to agent | Approved nonproduction user group | Session 03 agent endpoint | Existing agent consumer access | Project administration |
...
Keep the Session 03 version pinned while testing. If either check fails, leave the stable endpoint
unchanged. If the candidate is already active, restore the previous Session 03 version at 100%
before changing infrastructure.

Then remove the Foundry project connection only when no other governed tool uses it. Preview the
approved APIM change path and delete the marked Session 04 MCP resources. Revoke the APIM backend

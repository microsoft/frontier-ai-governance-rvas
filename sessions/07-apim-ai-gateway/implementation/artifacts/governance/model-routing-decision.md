...
This record defines the routing decision for the Session 07 APIM gateway. APIM remains the source
...
| Consumer | The API product owner uses it to approve routing changes. The platform owner applies them through `gateway-control.json` and the Session 07 deployment path. |
...
the approved workload allowance by region in Session 15. Do not assume one shared global counter.

...
validates the client token and product subscription, applies the approved limits and Content Safety
policy, then uses its managed identity to call the pinned Foundry agent.

The result is a marked, version-controlled APIM API and product with body-free telemetry. We will
confirm that APIM rejects an invalid bearer identity before it calls Content Safety or Foundry.

### Why it matters

The API product owner gets one place to manage workload access and limits. Client authorization
stays separate from the identity used for the backend call. Operations gets correlation and token
metrics without storing prompts or responses.

### Boundaries

The deployment changes child resources in the approved nonproduction APIM instance. APIM stores
the live route and policy. Foundry stores the agent configuration. The control covers requests sent
through this route; owners must govern direct Foundry access separately.

Production ingress, semantic caching, secondary-region routing, and write-capable agents need
separate designs. [Session 08](../../08-api-center-ai-mcp-inventory/implementation/README.md)
...
Foundry token from its system-assigned managed identity and calls the pinned Session 05 agent.
...
The repository defines the gateway configuration applied by the deployment scripts. APIM is
authoritative for the live route, product, backends, and policy. Foundry remains authoritative for
the agent.
...
|---|---|---|---|---|
| Client access | Entra application token plus one APIM subscription per workload | Identity and usage allocation can be revoked separately | Clients manage two credentials | Entra alone can meet quota and revocation needs |
...
| Telemetry | Correlation and token metrics; body logging disabled | Supports operations without retaining content | Logs cannot explain a failed exchange from its content | A data owner approves narrow content capture |
...
disabled. Enable it only after the service and cost owners approve a Responses-compatible endpoint
with matching model behavior, agent version, data boundary, safety policy, logging dimensions, and
...
metrics, and streaming limits use estimates. Azure Cost Management and the invoice remain the
billing records.
...
Preflight checks the files, approved scope, APIM tier and identity, both role assignments, Content
Safety backend, logger, agent URL, existing API marker, and ARM `what-if`. Stop on any gate listed
above.
...
The script reruns preflight and deploys the marked API, product, named values, backend pool, policy,
and body-free diagnostics. Use the subscription issued before the session.
...
Run this configuration only against the nonproduction resources named in the deployment inputs and
routing decision.

To remove the route, first confirm that no approved consumer uses it. Through the approved APIM
change path, check `implementationSession=07-apim-ai-gateway` and remove only the Session 07 API,
product, backends, and four nonsecret named values. Leave APIM, the logger, Content Safety, Foundry,
role assignments, and repository definitions in place.

To disable an approved secondary while keeping the primary live, set `secondaryBackendEnabled` to
`false`, clear `SESSION06_SECONDARY_AGENT_BASE_URL`, rerun preflight, and redeploy. For permanent
retirement, identity reviews both role assignments and removes one only when no other approved APIM
route uses it.

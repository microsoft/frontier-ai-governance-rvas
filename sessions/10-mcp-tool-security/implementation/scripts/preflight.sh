...
if ((${#unresolved_sentinels[@]} > 0)); then
  ...
  fail "Resolve every Session 10 customer decision before deployment: ${unresolved_sentinels[*]}"
fi
...
for record in (environment, binding):
    if record.get('implementationSession') != '10-mcp-tool-security':
...
for required in ['validate-azure-ad-token', 'rate-limit-by-key', 'authentication-managed-identity', 'X-Correlation-ID', 'session10-mcp-tool-security']:
...
[[ "$API_STATUS" == '200' ]] || fail 'Unable to read the existing Session 05 policy assistant.'
[[ $(jq -r '.agent_card.description // ""' "$API_BODY_FILE") == *'05-governed-agent-baseline'* ]] || fail 'The approved agent ID is not the marked Session 05 policy assistant.'
...
if [[ -n "$existing_mcp" ]]; then
  [[ $(jq -r '.properties.description // ""' <<<"$existing_mcp") == *'implementationSession=10-mcp-tool-security'* ]] || fail 'An APIM API already uses the MCP server ID without the Session 10 marker.'
fi
...
az bicep build --file "$bicep_path" --stdout >/dev/null || fail 'The Session 10 Bicep definition failed to compile.'
az deployment group what-if --name session10-mcp-preview --resource-group "$(jq -r '.resourceGroupName' <<<"$environment_json")" --template-file "$bicep_path" --parameters "apiManagementName=$(jq -r '.apiManagementName' <<<"$environment_json")" --only-show-errors --no-pretty-print >/dev/null || fail 'The Session 10 deployment preview failed.'

echo 'PASS: Session 10 Markdown security cases, one-tool boundary, identity scopes, payload-free telemetry, approved APIM and backend scopes, and deployment preview are ready.'

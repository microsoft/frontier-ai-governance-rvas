...
: <<'REMOVED_DUPLICATE'
  [[ "$bearer_token" != *$'\r'* && "$bearer_token" != *$'\n'* ]] || \
    || { echo "ERROR: CITADEL_SMOKE_BEARER_TOKEN cannot contain a line break." >&2; return 1; }
  :
    printf 'header = "Authorization: Bearer %s"\n' "$bearer_token"
    printf 'header = "Content-Type: application/json"\n'
    printf 'header = "x-citadel-smoke-mode: %s"\n' "$smoke_mode"
    printf 'header = "x-release-commit-sha: %s"\n' "$commit_sha_value"
    printf 'header = "traceparent: %s"\n' "$traceparent_value"
  } | (
    unset CITADEL_SMOKE_BEARER_TOKEN
    curl --config - \
      --silent \
      --show-error \
...
[[ -n "${CITADEL_SMOKE_URL:-}" ]] || { echo "ERROR: required environment variable 'CITADEL_SMOKE_URL' is missing." >&2; exit 1; }
[[ -n "${CITADEL_SMOKE_FAILURE_URL:-}" ]] || { echo "ERROR: required environment variable 'CITADEL_SMOKE_FAILURE_URL' is missing." >&2; exit 1; }
[[ -n "${CITADEL_AI_RESOURCE_ID:-}" ]] || { echo "ERROR: required environment variable 'CITADEL_AI_RESOURCE_ID' is missing." >&2; exit 1; }
[[ -n "${CITADEL_LOG_ANALYTICS_WORKSPACE_ID:-}" ]] || { echo "ERROR: required environment variable 'CITADEL_LOG_ANALYTICS_WORKSPACE_ID' is missing." >&2; exit 1; }
[[ -n "${CITADEL_SMOKE_BEARER_TOKEN:-}" ]] || { echo "ERROR: required environment variable 'CITADEL_SMOKE_BEARER_TOKEN' is missing." >&2; exit 1; }
poll_timeout_seconds="${CITADEL_SMOKE_TIMEOUT_SECONDS:-180}"
poll_retry_seconds="${CITADEL_SMOKE_RETRY_SECONDS:-15}"
[[ "$poll_timeout_seconds" =~ ^[0-9]+$ ]] && (( poll_timeout_seconds >= 30 && poll_timeout_seconds <= 600 )) \
  || { echo "ERROR: CITADEL_SMOKE_TIMEOUT_SECONDS must be an integer from 30 through 600." >&2; exit 1; }
[[ "$poll_retry_seconds" =~ ^[0-9]+$ ]] && (( poll_retry_seconds >= 5 && poll_retry_seconds <= 60 )) \
  || { echo "ERROR: CITADEL_SMOKE_RETRY_SECONDS must be an integer from 5 through 60." >&2; exit 1; }
...
poll_timing_valid "$poll_timeout_seconds" "$poll_retry_seconds" \
  || { echo "ERROR: CITADEL_SMOKE_TIMEOUT_SECONDS must be at least twice CITADEL_SMOKE_RETRY_SECONDS." >&2; exit 1; }

[[ "$CITADEL_AI_RESOURCE_ID" =~ ^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft\.Insights/components/[^/]+$ ]] \
  || { echo "ERROR: CITADEL_AI_RESOURCE_ID must be a full Application Insights resource ID." >&2; exit 1; }
[[ "$CITADEL_LOG_ANALYTICS_WORKSPACE_ID" =~ ^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft\.OperationalInsights/workspaces/[^/]+$ ]] \
  || { echo "ERROR: CITADEL_LOG_ANALYTICS_WORKSPACE_ID must be a full Log Analytics workspace resource ID." >&2; exit 1; }
[[ "$CITADEL_SMOKE_URL" == https://* && "$CITADEL_SMOKE_FAILURE_URL" == https://* ]] \
  || { echo "ERROR: both smoke endpoints must use HTTPS." >&2; exit 1; }
[[ "$CITADEL_SMOKE_URL" != "$CITADEL_SMOKE_FAILURE_URL" ]] \
...
[[ "${component_type,,}" == "microsoft.insights/components" ]] \
  || { echo "ERROR: CITADEL_AI_RESOURCE_ID could not be resolved as an Application Insights component." >&2; exit 1; }
workspace_binding_matches "$component_workspace_resource_id" "$CITADEL_LOG_ANALYTICS_WORKSPACE_ID" \
  || { echo "ERROR: the Application Insights component WorkspaceResourceId does not match CITADEL_LOG_ANALYTICS_WORKSPACE_ID." >&2; exit 1; }
normalized_workspace_resource_id="$(normalize_resource_id "$CITADEL_LOG_ANALYTICS_WORKSPACE_ID")"
...
synthetic_marker="citadel-probe-$(python3 -c 'import secrets; print(secrets.token_hex(16))')"
...
jq -n \
  --arg implementation_session "06-citadel-observability-operations" \
  --arg mode "$mode" \
  ...
  '{
    ...
    recordType: "citadel-smoke-result",
    mode: $mode,
...
[[ "$status" == "passed" ]] || { echo "ERROR: Session 06 smoke checks failed. Inspect the payload-free result at $result_path." >&2; exit 1; }
echo "PASS: Session 06 smoke checks passed."
}

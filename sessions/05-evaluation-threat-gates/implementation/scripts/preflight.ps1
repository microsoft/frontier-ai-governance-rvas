...
# Red-team plan sentinels:
# __REQUIRED_TAXONOMY_NAME__ __REQUIRED_SECURITY_OWNER_ROLE__
...
$implementationSession = "05-evaluation-threat-gates"
...
if ($sentinels.Count -gt 0) {
    ...
    throw "Resolve every Session 05 customer decision before running evaluation: $($unresolved -join ', ')."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Install the Session 05 Python dependencies: python -m pip install -r `"$requirementsPath`""
}
...
Write-Host "PASS: Session 05 release policy, current manual support gate, Foundry target, dataset, dependencies, and $Phase gate are ready."

# SOC triage playbook for Session 05 AI signals
...
- For unsafe tool behavior, disable the agent endpoint or remove the tool binding through the
  existing Session 03 agent or Session 04 contract control. Do not depend on a system-prompt edit
  alone.
...
Recovery returns only the previously approved agent version and tool boundary. A Session 05 result
...
surface: supported blocking depends on the agent type and integration. Session 05 does not create

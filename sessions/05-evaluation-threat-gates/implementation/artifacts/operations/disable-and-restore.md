...
Keep or restore the stable endpoint at 100% of the Session 03 approved fixed agent version. Do not pin the
candidate version if either evaluation path fails or the release owner cannot inspect the result.
The Session 05 scripts never change the endpoint selector.
...
3. Remove the Session 05 gate from a delivery workflow only after the release owner confirms that no
   active promotion depends on it.
4. Cancel any running Session 05 evaluation from Foundry if it is consuming unnecessary budget.
...
Session 04 tool path, or customer data as a restore shortcut. A failed candidate stays unpinned

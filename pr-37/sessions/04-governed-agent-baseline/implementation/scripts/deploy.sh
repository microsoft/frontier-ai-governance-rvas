...
if [[ "$API_STATUS" == '200' ]]; then
  ...
  [[ "$existing_description" == *'04-governed-agent-baseline'* ]] || fail "An agent with this name exists without the Session 04 implementation marker."
elif [[ "$API_STATUS" != '404' ]]; then
  fail "Unable to inspect the configured Session 04 agent name."
fi
...
jq -n \
  ...
  --arg session "04-governed-agent-baseline" \

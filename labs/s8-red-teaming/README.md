...
This lab helps the customer make one bounded adversarial-testing decision and hand it to the right owner. The facilitator guides the method; the customer inspects its own Microsoft records, chooses the decision, and keeps completed evidence in its approved records system.
...
By the end of the lab, the customer has inspected the red-team plan/run record, PyRIT or AI Red Teaming Agent finding, Azure AI Content Safety result, Defender/Sentinel case, and remediation owner record; recorded approve, defer, reject, or route; created backlog for gaps; and documented any exception.
...
1. Inspect the Microsoft control path: **AI Red Teaming Agent, PyRIT, Azure AI Content Safety, Defender, and SOC remediation routes**.
...
6. Create a red-team remediation backlog item for each confirmed finding, missing safety control, unowned risk, retest requirement, or SOC escalation route.
...
When completing the shared decision record, capture the technical decision record reference for the authorized adversarial test, finding interpretation, remediation route, and retest plan.

## Handoff

Handoff to red team lead, safety owner, SOC, product owner, and release manager. The receiving owner accepts only backlog items with clear acceptance tests, target dates, and evidence locations. Keep final records in the customer-approved system.

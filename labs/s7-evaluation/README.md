...
- **Gate handoff:** pipeline or manual gate mode, release/hold process, diagnostic-only limits if S6/S3 prerequisites are missing, and Azure Load Testing evidence when performance affects the decision.
- **Blockers and backlog:** missing scenario set, evaluator version, threshold owner, baseline, evidence location, load-test record, S6 runtime prerequisite, release/hold owner, access, license, or scope clarity captured with owner, acceptance test, target date, evidence location, and review trigger.
...
7. Create an evaluation backlog item for each missing scenario, scorer/rubric version, baseline, threshold owner, load-test record, CI/CD gate, S6/S3 prerequisite, exception owner, or release/hold decision path.
...
- **Blocked** when access, licensing, evidence location, ownership, S6/S3 prerequisites, evaluator availability, baseline, or scope clarity prevents a decision.
